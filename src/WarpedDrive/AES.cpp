...
static uint8_t AES_CONSTANT[10] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 };

static uint8_t AES_FORWARD_BOX[256] = {
...
static uint8_t AES_REVERSE_BOX[256] = {
...
static uint32_t AES_FORWARD_TABLE[4][256] = {
    {
...
static uint32_t AES_REVERSE_TABLE[4][256] = {
    {
...
AESCipher::AESCipher(uint8_t* key, uint32_t cbKey, bool encrypt) : _encrypt(encrypt), _cycles(6 + cbKey / 4)
{
    if (encrypt)
    {
        _init_encrypt(key, cbKey);
    }
    else
    {
        _init_decrypt(key, cbKey);
    }
}


AESCipher::AESCipher(uint8_t* key, uint32_t cbKey, uint8_t* tweakKey, uint32_t cbTweakKey, bool encrypt) : AESCipher(key, cbKey, encrypt)
{
    _tweak.reset(new AESCipher(tweakKey, cbTweakKey, true));
}


void AESCipher::_init_encrypt(uint8_t* key, uint32_t cbKey)
{
    uint8_t     rounds = 0;
    uint8_t     increment = 0;
    uint8_t     offset = 0;
    aesint_t*   round_key = NULL;
    aesint_t    rotate = 0;

    offset = _cycles - 6;
    increment = 4 + (offset % 4);
    rounds = _cycles;
    if (cbKey == 24)
    {
        rounds = 8;
    ...
    memcpy(_round_key, key, cbKey);

    round_key = _round_key;
    for (uint8_t cycle = 0; cycle < rounds; cycle++)
    {
        round_key[offset] = round_key[0];

        if (cbKey < 32)
        {
            round_key[offset] ^= AES_CONSTANT[cycle];
        }
        else if (0 == cycle % 2)
        {
            round_key[offset] ^= AES_CONSTANT[cycle / 2];
        }

        rotate = round_key[offset - 1].rotate();
        round_key[offset] ^= aesint_t(AES_FORWARD_BOX[rotate[0]], AES_FORWARD_BOX[rotate[1]], AES_FORWARD_BOX[rotate[2]], AES_FORWARD_BOX[rotate[3]]);

        for (uint8_t i = 1; i < increment; i++)
        {
            round_key[offset + i] = round_key[i] ^ round_key[offset + i - 1];
        }
...
void AESCipher::_init_decrypt(uint8_t* key, uint32_t cbKey)
{
    AESCipher   encrypt(key, cbKey, true);
    aesint_t*   encrypt_key = NULL;
    aesint_t*   round_key = NULL;

    encrypt_key = encrypt._round_key + _cycles * 4;
    round_key = _round_key;

    for (uint8_t i = 0; i < 4; i++)
    {
        *(round_key++) = encrypt_key[i];
    }
    encrypt_key -= 4;

    for (uint8_t cycle = _cycles; cycle > 1; cycle--, encrypt_key -= 4)
    {
        for (uint8_t i = 0; i < 4; i++, round_key++)
        {
            *round_key  = AES_REVERSE_TABLE[0][AES_FORWARD_BOX[encrypt_key[i][0]]] ^ \
                          AES_REVERSE_TABLE[1][AES_FORWARD_BOX[encrypt_key[i][1]]] ^ \
                          AES_REVERSE_TABLE[2][AES_FORWARD_BOX[encrypt_key[i][2]]] ^ \
                          AES_REVERSE_TABLE[3][AES_FORWARD_BOX[encrypt_key[i][3]]];
        }
    }
...
uint32_t AESCipher::ecb(const uint8_t* inputData, uint32_t cbInput, uint8_t* outputData, uint32_t cbOutput)
{
    uint32_t*   input = (uint32_t*)inputData;
    uint32_t*   output = (uint32_t*)outputData;
    uint32_t    (*table)[256] = NULL;
    uint8_t*    box = NULL;
    uint8_t     increment = 0;
    aesint_t    values[4] = { 0 };
    aesint_t    cipher[4] = { 0 };
    ...
    if (_encrypt)
    {
        table = AES_FORWARD_TABLE;
        box = AES_FORWARD_BOX;
        increment = 1;
    }
    else
    {
        table = AES_REVERSE_TABLE;
        box = AES_REVERSE_BOX;
        increment = 3;
    }

    for (index = 0; (index < (cbInput / 4)) && (index < (cbOutput / 4)); index += 4)
    {
        uint32_t    round = 0;

        for (uint8_t i = 0; i < 4; i++)
        {
            values[i] = input[index + i] ^ _round_key[round++];
        }

        for (uint32_t cycle = (_cycles / 2); cycle > 1; cycle--)
        {
            for (uint8_t i = 0; i < 4; i++)
            {
                cipher[i] = table[0][values[(i + (0 * increment)) % 4][0]] ^ \
                            table[1][values[(i + (1 * increment)) % 4][1]] ^ \
                            table[2][values[(i + (2 * increment)) % 4][2]] ^ \
                            table[3][values[(i + (3 * increment)) % 4][3]] ^ \
                            _round_key[round++];
            }
            for (uint8_t i = 0; i < 4; i++)
            {
                values[i] = table[0][cipher[(i + (0 * increment)) % 4][0]] ^ \
                            table[1][cipher[(i + (1 * increment)) % 4][1]] ^ \
                            table[2][cipher[(i + (2 * increment)) % 4][2]] ^ \
                            table[3][cipher[(i + (3 * increment)) % 4][3]] ^ \
                            _round_key[round++];
            }
        }
        ...
        for (uint8_t i = 0; i < 4; i++)
        {
            values[i] = aesint_t(box[cipher[(i + (0 * increment)) % 4][0]],
                                 box[cipher[(i + (1 * increment)) % 4][1]],
                                 box[cipher[(i + (2 * increment)) % 4][2]],
                                 box[cipher[(i + (3 * increment)) % 4][3]]) ^ \
                        _round_key[round++];
        }
...
void aes_update_tweak(uint8_t tweak[16])
{
    uint8_t carry_bit = 0;
    uint8_t swap = 0;
    uint8_t i = 0;
...
uint32_t AESCipher::xts(const uint8_t* inputData, uint32_t cbInput, const uint8_t tweakData[16], uint8_t* outputData, uint32_t cbOutput)
{
    uint8_t     tweak[16] = { 0 };
    uint8_t     tweak_copy[16] = { 0 };
    uint32_t    offset = 0;

    if (!_tweak.get()) return 0;
    _tweak.get()->ecb(tweakData, 16, tweak, 16);

    for (offset = 0; (offset + 16) <= cbInput; offset += 16)
    {
        if (!_encrypt && (32 > (cbInput - offset)) && (16 != (cbInput - offset)))
        {
            memcpy(tweak_copy, tweak, 16);
            aes_update_tweak(tweak);
        }

        for (uint8_t i = 0; i < 16; i++)
        {
            outputData[offset + i] = inputData[offset + i] ^ tweak[i];
        }

        ecb(outputData + offset, 16, outputData + offset, 16);

        for (uint8_t i = 0; i < 16; i++)
        {
            outputData[offset + i] ^= tweak[i];
        }
    ...
    if (cbInput > offset)
    {
        if (!_encrypt)
        {
            memcpy(tweak, tweak_copy, 16);
        }

        offset -= 16;
        memcpy(outputData + offset + 16, outputData + offset, cbInput - offset);
        memcpy(outputData + offset, inputData + offset + 16, cbInput - offset);
